...
export const CustomerDetailPage: React.FC = () => {
  ...
  return (
    <div className="container py-6 space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          <Link
            ...
            className="p-2 -ml-2 rounded-sm hover:bg-accent"
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {customer.isPassive ? (
            <span
              className="inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded-sm bg-muted text-foreground"
              title={t(
      ...
      {/* Account section */}
      <Card className="py-8"><CardContent className="px-8"><h2 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                    <Mail className="w-5 h-5" /> {t('customers.detail.accountSection', 'Account')}
                  </h2><div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.email', 'Email')}</label>
                      <Input type="email" value={form.email || ''} onChange={setField('email')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.preferredLanguage', 'Preferred language')}</label>
                      <select
      ...
      {/* Personal section */}
      <Card className="py-8"><CardContent className="px-8"><h2 className="text-lg font-semibold text-foreground mb-4">
                    {t('customers.detail.personalSection', 'Personal information')}
                  </h2><div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.salutation', 'Salutation')}</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.firstName', 'First name')}</label>
                      <Input value={form.firstName || ''} onChange={setField('firstName')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.lastName', 'Last name')}</label>
                      <Input value={form.lastName || ''} onChange={setField('lastName')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.displayName', 'Display name')}</label>
      ...
      <Card className="py-8"><CardContent className="px-8"><h2 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                  ...
                  </h2><p className="text-xs text-muted-foreground mb-3">
                    ...
                    <ul className="divide-y divide-neutral-200 dark:divide-neutral-700">
                      {customer.events.map((ev) => (
                        <li key={ev.id} className="py-2 flex items-center justify-between">
                          <Link to={`/admin/events/${ev.id}`} className="text-foreground hover:underline">
                            {ev.eventName}
                          </Link>
      ...
      <Card className="py-8"><CardContent className="px-8"><h2 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.billingEmail', 'Billing email')}</label>
                      <Input type="email" value={form.billingEmail || ''} onChange={setField('billingEmail')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.vatId', 'VAT / tax ID')}</label>
                      <Input value={form.vatId || ''} onChange={setField('vatId')} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.addressLine1', 'Address line 1')}</label>
                      <Input value={form.addressLine1 || ''} onChange={setField('addressLine1')} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.addressLine2', 'Address line 2')}</label>
                      <Input value={form.addressLine2 || ''} onChange={setField('addressLine2')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.postalCode', 'Postal code')}</label>
                      <Input value={form.postalCode || ''} onChange={setField('postalCode')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.city', 'City')}</label>
                      <Input value={form.city || ''} onChange={setField('city')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t('customers.detail.state', 'State / region')}</label>
      ...
      <Card className="py-8"><CardContent className="px-8"><h2 className="text-lg font-semibold text-foreground mb-1 flex items-center gap-2">
                        ...
                        {(form.billingCadence === 'monthly' || form.billingCadence === 'quarterly') && (
                          ...
                          once instead of ticking the per-invoice toggle every time. */}<label className="mt-4 flex items-start gap-2 text-sm text-foreground">
                        <input
                          type="checkbox"
                              ...
                              <tbody>
                                {monthlyDraft.lineItems.map((li) => (
                                  <tr key={li.id} className="border-t border-border">
                                    <td className="px-3 py-1.5 tabular-nums text-muted-foreground">{li.position}</td>
