    ...
    <div className="container py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-foreground">{t('quotes.title', 'Quotes')}</h1>
      ...
      <Card className="py-8"><CardContent className="px-8"><div className="flex flex-wrap items-center gap-3">
                    ...
                    {STATUSES.map((s) => {
                      ...
                      single-card layout used by Customers/Invitations. */}<div className="mt-4">
                    {isLoading ? <Loading /> : !data || data.quotes.length === 0 ? (
                      <p className="text-center text-muted-foreground py-8">{t('quotes.empty', 'No quotes yet.')}</p>
                    ) : (
