  ...
  return (
    <div className="space-y-4">
                    ...
                    {draft.sections.map((section, idx) => (
                      <div key={idx} className="rounded-lg border border-border p-3">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{sectionTitle(section)}</span>
                          <div className="flex items-center gap-1">
                            ...
                            <button type="button" onClick={() => moveSection(idx, -1)} aria-label="Move up" className="p-1 rounded-sm hover:bg-accent"><ArrowUp className="w-4 h-4" /></button>
                            <button type="button" onClick={() => moveSection(idx, 1)} aria-label="Move down" className="p-1 rounded-sm hover:bg-accent"><ArrowDown className="w-4 h-4" /></button>
      ...
      {data.versions.length > 0 && (
        <Card className="py-8"><CardContent className="px-8"><h3 className="font-semibold mb-3">{t('quotes.templates.versions', 'Published versions')}</h3><ul className="text-sm space-y-1 text-foreground">
                          {data.versions.map((v) => (
