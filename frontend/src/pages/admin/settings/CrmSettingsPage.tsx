  ...
  return (
    <div className="space-y-4">
      ...
      <Card><CardContent><h3 className="font-semibold text-foreground mb-3">{t('crmSettings.section.quotes', 'Quotes')}</h3>{checkbox('crm_quotes_pdf_attachment_enabled', 'Attach quote PDF to email')}{checkbox('crm_quotes_skonto_enabled', 'Allow early-payment discount (Skonto) on quotes')}<div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                        ...
                        <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_quotes_default_valid_days.label', 'Default validity (days)') as string}</span><Input type="number" min={1} max={365}
                                            value={values.crm_quotes_default_valid_days ?? 30}
                                            onChange={(e) => setVal('crm_quotes_default_valid_days', Number(e.target.value))} /></Label></div>
                        <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_quotes_number_format.label', 'Quote number format') as string}</span><Input
                                            value={values.crm_quotes_number_format ?? ''}
                                            onChange={(e) => setVal('crm_quotes_number_format', e.target.value)} /></Label></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                          <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_quotes_tos_url.label', 'Terms of Service URL (optional)') as string}</span><Input
      ...
      <Card><CardContent><h3 className="font-semibold text-foreground mb-3">{t('crmSettings.section.invoices', 'Invoices')}</h3>{checkbox('crm_invoices_qr_enabled', 'Render payment QR on invoice PDFs')}{checkbox('crm_invoice_round_total', 'Reconcile sub-cent rounding to a clean total (adds a "Rundung" row when per-line rounding drifts from qty × rate)')}{/* Free-text VAT / legal note (#794) — printed directly under the MwSt.
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          ...
                          the dunning path applies, not part of the schedule. */}{workflowsLive ? (
                      ...
                      )}{checkbox('crm_invoices_late_fee_enabled', 'Add a late fee (Mahngebühr) on every reminder after the first')}<div className="mt-2 rounded-lg border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 p-3 text-sm text-amber-800 dark:text-amber-200">
                      ...
                      </div>{checkbox('crm_invoices_late_fee_vat_enabled', 'Charge VAT on late fees (Switzerland — leave off for DE/AT; no effect if your organisation has no VAT rate)')}<div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                        {!workflowsLive && (
                          <>
                            <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_reminder_first_days.label', 'First reminder after (days past due)') as string}</span><Input type="number" min={1} max={365}
                                                            value={values.crm_invoices_reminder_first_days ?? 14}
                                                            onChange={(e) => setVal('crm_invoices_reminder_first_days', Number(e.target.value))} /></Label></div>
                            <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_reminder_second_days.label', 'Second reminder after (days past due)') as string}</span><Input type="number" min={1} max={365}
                                                            value={values.crm_invoices_reminder_second_days ?? 30}
                                                            onChange={(e) => setVal('crm_invoices_reminder_second_days', Number(e.target.value))} /></Label></div>
                          </>
                        )}
                        <div>
                          <label className="block text-sm font-medium text-foreground mb-1">
                        ...
                        {(values.crm_invoices_late_fee_type ?? 'flat') === 'percent' ? (
                          <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_late_fee_percent.label', 'Late fee (% of invoice)') as string}</span><Input type="number" min={0} step="0.01" max={100}
                                                      value={values.crm_invoices_late_fee_percent ?? 0}
                                                      onChange={(e) => setVal('crm_invoices_late_fee_percent', Number(e.target.value))} /></Label></div>
                        ) : (
                          <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_late_fee_minor.label', 'Late fee (minor units / Rappen)') as string}</span><Input type="number" min={0}
                                                          value={values.crm_invoices_late_fee_minor ?? 0}
                                                          onChange={(e) => setVal('crm_invoices_late_fee_minor', Number(e.target.value))} /></Label></div>
                        )}
                        <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_late_fee_label.label', 'Late fee label') as string}</span><Input
                                                value={values.crm_invoices_late_fee_label ?? 'Mahngebühr'}
                                                onChange={(e) => setVal('crm_invoices_late_fee_label', e.target.value)} /></Label></div>
                        <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_skonto_percent_default.label', 'Skonto rate (default %)') as string}</span><Input type="number" min={0} step="0.01" max="100"
                                                value={values.crm_invoices_skonto_percent_default ?? 2}
                                                onChange={(e) => setVal('crm_invoices_skonto_percent_default', Number(e.target.value))} /></Label></div>
                        <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_skonto_business_days.label', 'Skonto window (business days)') as string}</span><Input type="number" min={0}
                                                value={values.crm_invoices_skonto_business_days ?? 5}
                                                onChange={(e) => setVal('crm_invoices_skonto_business_days', Number(e.target.value))} /></Label></div>
                        <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_number_format.label', 'Invoice number format') as string}</span><Input
                                                value={values.crm_invoices_number_format ?? ''}
                                                onChange={(e) => setVal('crm_invoices_number_format', e.target.value)} /></Label></div>
                      </div>{/* Installment defaults (migration 141). Pre-populate fresh
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          ...
                          <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_installment_days_before_event.label', 'Days before event (middle installment)') as string}</span><Input type="number" min={0}
                                                      value={values.crm_invoices_installment_days_before_event ?? 14}
                                                      onChange={(e) => setVal('crm_invoices_installment_days_before_event', Number(e.target.value))} /></Label></div>
                          <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_invoices_installment_days_after_event.label', 'Days after event (final installment)') as string}</span><Input type="number" min={0}
                                                      value={values.crm_invoices_installment_days_after_event ?? 14}
                                                      onChange={(e) => setVal('crm_invoices_installment_days_after_event', Number(e.target.value))} /></Label></div>
                        </div>
                      </div>{/* Pre-event customer reminders moved to their own dedicated
      ...
      <Card><CardContent><h3 className="font-semibold text-foreground mb-3">{t('crmSettings.section.contracts', 'Contracts')}</h3>{/* Three of these four toggles use `!== false` semantics on
                        ...
                        <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_contracts_default_valid_days.label', 'Signing window (days)') as string}</span><Input type="number" min={1} max={365}
                                                value={values.crm_contracts_default_valid_days ?? 30}
                                                onChange={(e) => setVal('crm_contracts_default_valid_days', Number(e.target.value))} /></Label></div>
                        <div className="w-full"><Label className="block"><span className="mb-1.5 block">{t('crmSettings.crm_contracts_number_format.label', 'Contract number format') as string}</span><Input
