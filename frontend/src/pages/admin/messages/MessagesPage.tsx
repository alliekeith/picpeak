  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8.5rem)] min-h-[540px]">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="relative flex-1 max-w-md ml-auto">
          ...
          <input
            ...
            className="w-full h-9 pl-9 pr-3 rounded-lg border border-border bg-muted text-sm text-foreground focus:outline-hidden focus:ring-2 focus:ring-brand"
          />
        </div>
        <div className="flex items-center gap-2 flex-none">
          <button
            ...
            className="inline-flex items-center gap-2 h-9 px-3 rounded-lg border border-border text-sm font-medium text-foreground hover:bg-accent disabled:opacity-60"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 h-9 px-3.5 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90"
          >
      ...
      <div className="flex flex-1 min-h-0 rounded-xl border border-border overflow-hidden bg-card">
        {/* ── account tree ── */}
        <nav className="w-56 flex-none border-r border-border overflow-y-auto p-2 bg-muted">
          {accounts.map((a) => (
            <div key={a.id} className="mb-1.5">
              <div className="flex items-center gap-2 px-2 py-1.5 text-sm font-semibold text-foreground">
                ...
                <span>{a.name}</span>
                {a.addr && <span title={a.addr} className="ml-auto text-[11px] font-medium font-mono text-muted-foreground truncate max-w-28">{localPart(a.addr)}</span>}
              </div>
                ...
                {a.folders.map((f) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span>{f.name}</span>
                      {typeof c === 'number' && c > 0 && (
                        <span className={`ml-auto tabular-nums text-xs ${active ? 'text-on-brand-soft' : 'text-muted-foreground'}`}>{c}</span>
                      )}
                    </button>
                  );
                })}
        ...
        {/* ── message list ── */}
        <section className="w-[22rem] flex-none flex flex-col min-h-0 border-r border-border">
          <div className="px-4 py-3 border-b border-border flex-none">
            <div className="text-base font-semibold text-foreground">{folder.f.name}</div>
...
const MessageList: React.FC<{
  ...
  if (rows.length === 0) {
    return <div className="p-8 text-center text-sm text-muted-foreground">{q ? t('messages.noSearchResults', 'No matches') : t('messages.noMessages', 'No messages')}</div>;
  }
    ...
    <ul>
      {rows.map((r) => (
        <li key={r.key}>
          <button
            onClick={r.onClick}
            className={`w-full text-left px-4 py-3 border-b border-border border-l-[3px] transition-colors ${
              r.active
          ...
          >
            <div className="flex items-center gap-2">
              <span className="font-semibold text-[13.5px] text-foreground truncate">{r.who}</span>
              <span className="ml-auto text-[11px] text-muted-foreground tabular-nums whitespace-nowrap">{r.when}</span>
            </div>
            <div className="text-[13px] text-muted-foreground truncate mt-0.5">{r.subject}</div>
            <div className="flex items-center gap-2 mt-1.5">
              <span className={`text-[10.5px] font-semibold px-1.5 py-0.5 rounded-full ${STATUS_STYLES[r.status] || 'bg-muted text-muted-foreground'}`}>
                {r.status}
              </span>
              {r.attach > 0 && (
...
const ReadingPane: React.FC<{
  ...
  if (!selection) {
    return (
      <div className="flex-1 grid place-items-center text-center text-muted-foreground p-10">
        <div>
          <Mail className="w-9 h-9 mx-auto mb-3 text-muted-foreground" />
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {selection.kind === 'queue' ? (
          detailQuery.isLoading ? <Loading /> : detailQuery.data ? (
            ...
            <div className="text-sm text-muted-foreground">{t('messages.loadError', 'Could not load this message.')}</div>
          )
        ) : (
          <ReceivedDetail
            item={selection.item}
...
const QueueDetail: React.FC<{ d: import('../../../services/email.service').EmailQueueDetail; fromAddr?: string | null; t: TFunction }> = ({ d, fromAddr, t }) => (
  <>
    <h2 className="text-xl font-semibold text-foreground" style={{ textWrap: 'balance' } as React.CSSProperties}>
      {friendlyType(d.emailType)}
    </h2>
    <div className="mt-3 pb-4 border-b border-border text-sm">
      <div className="text-muted-foreground">
        {t('messages.from', 'from')} <span className="font-mono text-xs">{fromAddr || '—'}</span> · {t('messages.to', 'to')}{' '}
        <span className="font-semibold text-foreground">{d.recipientEmail}</span>
      </div>
      {d.cc && <div className="text-muted-foreground text-xs mt-0.5">cc {d.cc}</div>}
      <div className="text-muted-foreground text-xs mt-0.5 tabular-nums">{fmt(d.sentAt || d.createdAt)}</div>
    </div>

    {d.renderedHtml ? (
      <div className="mt-4 rounded-lg border border-border overflow-hidden bg-card" style={{ height: '52vh' }}>
      ...
      <div className="mt-5">
        ...
        <div className="flex flex-col gap-2 max-w-md">
          {d.attachments.map((a, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-border bg-muted">
              <FileText className="w-5 h-5 text-red-500 flex-none" />
              <span className="text-[13.5px] font-medium text-foreground truncate">{a.filename}</span>
              <span className="ml-auto text-[11px] text-muted-foreground" title={t('messages.sentAttachHint', 'Sent attachments are not archived yet — Phase 2.')}>
...
const ReceivedDetail: React.FC<{
  ...
  return (
    <>
      ...
      <div className="mt-3 pb-4 border-b border-border text-sm">
        <div className="text-muted-foreground">
          {t('messages.from', 'from')} <span className="font-semibold text-foreground">{item.from_address || '—'}</span>
          {' · '}{t('messages.to', 'to')} <span className="font-mono text-xs">{toAddr}</span>
        </div>
        <div className="text-muted-foreground text-xs mt-0.5 tabular-nums">{fmt(item.received_at)}</div>
      </div>

      {detail.isLoading ? (
        <div className="mt-4"><Loading /></div>
      ) : detail.data?.body_html ? (
        <div className="mt-4 rounded-lg border border-border overflow-hidden bg-card" style={{ height: '48vh' }}>
        ...
        <pre className="mt-4 whitespace-pre-wrap text-sm text-foreground font-sans">{detail.data.body_text}</pre>
      ) : (
        <div className="mt-4 text-sm text-muted-foreground italic">
      ...
      {item.inbound_document_id != null && (
        <div className="mt-5 flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg bg-primary hover:opacity-90 text-primary-foreground text-sm font-medium"
          >
            <FileText className="w-4 h-4" />{t('messages.viewDocument', 'View document')}
          </button>
          <button
            onClick={onOpenAccounting}
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg border border-border text-foreground text-sm font-medium hover:bg-accent"
          >
...
const Toolbar: React.FC<{
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap px-3 py-2 border-b border-border flex-none">
...
const PdfModal: React.FC<{ docId: number; onClose: () => void; t: TFunction }> = ({ docId, onClose, t }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/55 p-6" onClick={onClose}>
      <div className="bg-card rounded-xl w-[min(620px,94vw)] max-h-[90vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
          <FileText className="w-4 h-4 text-red-500" />
          <span className="text-sm font-medium text-foreground">{t('messages.document', 'Document')}</span>
          <div className="ml-auto flex items-center gap-1">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
              className="w-8 h-8 grid place-items-center rounded-lg text-muted-foreground hover:bg-accent disabled:opacity-40">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="text-xs tabular-nums text-muted-foreground w-6 text-center">{page}</span>
            <button onClick={() => setPage((p) => p + 1)}
              className="w-8 h-8 grid place-items-center rounded-lg text-muted-foreground hover:bg-accent">
              <ChevronRight className="w-4 h-4" />
            </button>
            <button onClick={onClose} aria-label={t('messages.close', 'Close')}
              className="w-8 h-8 grid place-items-center rounded-lg text-muted-foreground hover:bg-accent ml-1">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        <div className="overflow-auto p-5 bg-muted grid place-items-center min-h-[240px]">
          {err ? (
            <div className="text-sm text-muted-foreground">{t('messages.previewUnavailable', 'Preview unavailable')}</div>
          ) : url ? (
