...
export const FeaturesTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <Card><CardContent>{/* Header */}<div className="mb-6 pb-4 border-b border-neutral-200 dark:border-neutral-700">
                  ...
                  </div>{/* Core */}<Section title={t('settings.features.sections.core', 'Core')}>
                    <FeatureCard
                      icon={Images}
      ...
      <div className="flex items-center justify-end gap-2 pt-2">
        ...
        <Button
                        onClick={() => { void save(); }} disabled={!isDirty || isSaving || isSaving}
                      >
                        {isSaving && <Loader2 className="animate-spin" />}<Save className="w-4 h-4" />{t('common.saveChanges', 'Save changes')}</Button>
      </div>
    </div>
  );
};
