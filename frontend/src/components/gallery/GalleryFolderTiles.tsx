      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {tiles.map(({ category, count }) => (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-surface bg-surface text-sm hover:shadow-xs transition-shadow focus:outline-hidden focus:ring-2 focus:ring-primary-500"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {tiles.map(({ category, count, coverPhoto }) => (
          <button
            ...
            onClick={() => onOpen(folderKey(category))}
            aria-label={t('gallery.openFolder', 'Open folder {{name}}', { name: category.name })}
            className="group text-left rounded-lg overflow-hidden border border-surface bg-surface hover:shadow-md transition-all focus:outline-hidden focus:ring-2 focus:ring-primary-500"
          >
