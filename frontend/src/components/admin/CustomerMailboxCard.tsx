...
import { Save, Server, User, Lock, Eye, EyeOff, PlugZap, Inbox, Loader2 } from 'lucide-react';
  ...
  return (
    <Card className="mt-6"><CardContent><h2 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100 mb-1 flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" onClick={() => test.mutate()} className="whitespace-nowrap" disabled={!cfg.imap_host || !cfg.imap_user || test.isPending}>
                ...
                <Button onClick={() => save.mutate()} className="flex-1 min-w-48" disabled={save.isPending}>
                                    {save.isPending && <Loader2 className="animate-spin" />}<Save className="w-5 h-5" />{t('email.customerMailbox.save', 'Save Customer Mailbox')}</Button>
              </div>
