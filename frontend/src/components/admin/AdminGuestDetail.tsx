...
export const AdminGuestDetail: React.FC<AdminGuestDetailProps> = ({ eventId, guest, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-start justify-center overflow-y-auto p-4 pt-16">
      ...
      <div className="relative bg-card rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            {tab === 'commented' ? (
              commented.length === 0 ? (
                ...
                <div className="space-y-3">
                  {commented.map((c, idx) => (
                    <div key={idx} className="flex gap-3 p-3 bg-muted rounded-sm">
                      <AuthenticatedImage
