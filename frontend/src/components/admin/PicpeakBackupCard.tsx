...
import { Download, Upload, AlertTriangle, ShieldAlert, ExternalLink, CheckCircle2, Loader2 } from 'lucide-react';
  ...
  return (
    <Card className="py-8"><CardContent className="px-8"><h3 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100">
              ...
              <Button
                            ...
                            >
                              {downloading && <Loader2 className="animate-spin" />}<Download className="h-4 w-4" />{t('backup.picpeak.download', 'Download .picpeak')}</Button>
  ...
  return (
    <Card className="py-8"><CardContent className="px-8"><h3 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100">
              ...
              {t('backup.picpeak.restoreIntro', 'Upload a .picpeak taken from this or another instance. Restoring a SQLite backup onto a PostgreSQL instance is supported (the upgrade path); other engine combinations must match.')}
                      ...
                      >
                        <Upload className="h-4 w-4" />{t('backup.picpeak.chooseFile', 'Choose .picpeak file…')}</Button>{result && (
                ...
                <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-neutral-800">
                  ...
                  <div className="mt-6 flex justify-end gap-3">
                    ...
                    <Button
                                              ...
                                              >
                                                {restoring && <Loader2 className="animate-spin" />}{t('backup.picpeak.confirmRestore', 'Delete & restore')}</Button>
                  </div>
                </div>
              </div>
            )}</CardContent></Card>
